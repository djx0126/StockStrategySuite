...
import com.stockstrategy.statistic.data.tester.DateTester;
import com.stockstrategy.statistic.data.tester.DateTester.HisDataType;
 ...
 *
 */
public class SLevel4HisTester extends AbstractStrategyStatisticData {
	 ...
	 */
	private static final String myStatisticType = Constant.SLevel4HisTester;
	...
	public DataArray actualGenerate(String stockCode, String statisticType,
		...
		try {
			...
			for (int i = 1 ; i<ma5.size();i++)
			{
				...
				HisDataType oneDayData = DateTester.getOneDayData(ma5.getDate(i));
				if (oneDayData!=null){
					//System.out.println(ma5.getDate(i)+":"+"toSell "+oneDayData.toSell+" inHand "+oneDayData.inHand);
					if (Integer.valueOf(oneDayData.toSell)>=Integer.valueOf(oneDayData.inHand)){
						continue;
					}
				}
			...
			////positive case
				...
				if (tobuy){
					statisticArray.setValue(i, 0.4);
				}
				
			}
			
			for (int i = 1 ; i<ma5.size();i++){
				if (statisticArray.getValue(i-1)>0 && statisticArray.getValue(i-2)>-0.01 && statisticArray.getValue(i-2)<0.01){
					if (close.getValue(i)>=1.000*close.getValue(i-1) && close.getValue(i)<1.01*close.getValue(i-1)){
						statisticArray.setValue(i, 0.4);
					}else if (close.getValue(i)>=1.01*close.getValue(i-1) && close.getValue(i)<1.02*close.getValue(i-1)){
						statisticArray.setValue(i, 0.5);
					}else if (close.getValue(i)>=1.02*close.getValue(i-1)){
						statisticArray.setValue(i, 0.6);
					}
				}
			}
			
			for (int i = 1 ; i<ma5.size();i++){
				if (statisticArray.getValue(i-1)>0 && statisticArray.getValue(i-2)>0.00 ){
					if (close.getValue(i)>close.getValue(i-1)){
						statisticArray.setValue(i, 0.4);
					}
				}
			}
