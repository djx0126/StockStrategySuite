 ...
 *
 */
public class SLevel1b extends AbstractStrategyStatisticData {
	 ...
	 */
	private static final String myStatisticType = Constant.SLevel1b;
	...
	public DataArray actualGenerate(String stockCode, String statisticType,
		...
		try {
			...
			kdjk = dataMap.getDataArray(Constant.KDJK);
			kdjd = dataMap.getDataArray(Constant.KDJD);
			kdjj = dataMap.getDataArray(Constant.KDJJ);
			...
			for (int i = 1 ; i<ma5.size();i++)
			{
				...
				for (int j=i;j>=i-60;j--){
					if (Math.abs(dif.getValue(j))>absdif){
						absdif = Math.abs(dif.getValue(j));
					}
				}
				if ( dif.getValue(i)<0 && absdif<0.3f){
					continue;
				}
