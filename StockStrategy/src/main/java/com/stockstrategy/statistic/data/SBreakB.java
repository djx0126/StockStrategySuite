 ...
 *
 */
public class SBreakB extends AbstractStrategyStatisticData {
	 ...
	 */
	private static final String myStatisticType = Constant.SBreakB;
	...
	public DataArray actualGenerate(String stockCode, String statisticType,
		...
		try {
			...
			for (int i = 0 ; i<ma5.size();i++)
			{
				double toBuy = statisticArray.getValue(i);
				if (status == 0 && toBuy > 0.0001d){
					status = 1;
				}else if (status == 1 && toBuy < -0.0001d){
					status = 0;
				}else{
