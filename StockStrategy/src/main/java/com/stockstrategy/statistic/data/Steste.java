...
public class Steste extends AbstractStrategyStatisticData {
	...
	public DataArray actualGenerate(String stockCode, String statisticType,
		...
		Map<String, Steste.BreakData> buyData = new HashMap<>();
			...
			if (macd.getValue(i) > 0 && macd.getValue(i - 1) < 0  ) {
				...
				if (lastDeathCrossK > 0 && lastGoldenCross > 0 && !minorDeath && !minorGoldenCross) {
					if (i - lastGoldenCross <60 && i - lastGoldenCross > 12
						...
						DataArray indexMA60 = indexDataMap.getDataArray(Constant.MA60);

						DataArray indexMA30 = indexDataMap.getDataArray(Constant.MA30);

						int indexI = indexMA60.getIndexByDate(close.getDate(i));

						double maxDifRLimit = indexMA30.getValue(indexI) > indexMA60.getValue(indexI) ? 0.00328021314758674 : -0.11328021314758674;
...
//				if (currentMax > 1.2d * currentData.buyPrice && close.getValue(i) < 0.85d * currentMax) {
//					toSell = true;
//				}
